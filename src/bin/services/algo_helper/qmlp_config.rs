...
impl<B: Backend> MyQmlp<B> {
    pub(crate) fn new(device: &B::Device, in_features: usize, out_actions: usize) -> Self {
        let linear1 = nn::LinearConfig::new(in_features, 64).with_bias(true).init(device);
        ...
        let linear4 = nn::LinearConfig::new(16, 16).with_bias(true).init(device);
        let output  = nn::LinearConfig::new(16, out_actions).with_bias(true).init(device);
